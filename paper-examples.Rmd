...
**NOTE**:
To replicate the analyses proposed in the manuscript, please use the downloaded data at 
`data/oarj.rdata`.
Because bibliographic metadata change at high frequency, downloads made on different days could provide slightly different results (*e.g.*, number of citations, number of published articles, *etc.*).
The `oarj.rdata` file contains all the objects we needed for this analysis.
...
```{r n-grams, fig.show='hide', eval=FALSE}
options("oa_ngrams.message.curlv5" = TRUE)
ngrams_data <- oa_ngrams(unique(biblio_works$id), verbose = TRUE)
